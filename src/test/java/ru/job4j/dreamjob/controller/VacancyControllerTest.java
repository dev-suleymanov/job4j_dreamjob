...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyByNonExistentIdThenGetErrorPage() {
        var nonExistentId = 0;
        when(vacancyService.findById(nonExistentId)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, nonExistentId);
    ...
    @Test
    public void whenUpdateVacancyThrowsExceptionThenGetErrorPageWithMessage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    public void whenDeleteNonExistentVacancyThenGetErrorPage() {
        var nonExistentId = 0;
        when(vacancyService.deleteById(nonExistentId)).thenReturn(false);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, nonExistentId);
        var actualMessage = model.getAttribute("message");
